...
制作选定方案时，先记录大色块边界、主字标与徽标相对翼面的中心、宽高和朝向，区分测量值与估计值；用接近方案的镜头校准，再映射到真实网格。顺序为大色块 → 主图案 → 次级标识 → 材质与工程细节。

贴花覆盖率只表示图案是否落在模型上，不代表位置、面积与方案一致。不能为通过检查而随意缩小或内移，也不能用增加小细节掩盖大布局偏差。必要的布局校准复用一张参考和一张低成本渲染，纳入已有图片预算；发现大偏差先修正，证据不足则明确交付为待确认草稿。
